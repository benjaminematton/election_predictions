...
Most political-donor advice is bad. Small-dollar donors often back longshot challengers who lose by 30 points, while the close races they could actually move are crowded out by megadonors who don't need their $50. This project replicates [Oath](https://oath.vote)'s answer: a 1–10 Impact Score per Democratic House candidate, combining **competitiveness**, **chamber-control stakes**, and **financial need**. Backtested on held-out 2024 results with snapshot-date discipline (model sees only what was knowable at T-60 days before Election Day), competitiveness-driven recommendations direct **~91% of allocated dollars to <5%-margin races, statistically tied with Cook Political Report's final pre-election ratings** despite Cook having 53 more days of information at evaluation time.

The pre-resume audit ([Validity checks](#validity-checks) below) rewrote the headline — the prior "+60pp over Cook" claim was a benchmark allocation bug. The honest story is more interesting: a model with stale inputs holds even with Cook's expert ratings using current information.
...
- **Backtested with bootstrap CIs** on every claim; model, fundraising baseline, and corrected Cook benchmark all carry 95% percentile-bootstrap intervals.
...
- **Wikipedia ratings back-fill audit.** Queried MediaWiki page-creation timestamps for every cycle (2014, 2016, 2018, 2020, 2022, 2024). Found three back-filled cycles where the page was created after the cycle's election: 2014 and 2016 (created 2021-03-20) and **2018 (created 2018-12-29, 7 weeks post-election — added to `BACKFILLED_CYCLES` in this audit, was missing before).** 2020 / 2022 / 2024 are clean. 2014 was already dropped from training; 2016 is in the with-2016 headline variant.
- **Wayback Machine spike for 2016 ratings.** Hit the Wayback CDX API for `cookpolitical.com/house/charts/race-ratings`, found 100+ archived snapshots in the 2016 second half with within-day coverage of T-110/T-60/T-20. Parsed the 2016-T-60 page: 428 districts with contemporaneous ratings vs only 58 from the back-filled Wikipedia page. **Of 56 overlapping districts, 40 disagree on rating** — confirms the back-fill leak was material, not theoretical. Cached parquets staged at `data/raw/wayback_cook/`; full integration into the ingestion pipeline deferred (audit time-boxed at 4h kill).
...
| Snapshot | Model close-race [95% CI] | Fundraising [95% CI] | Cook-final [95% CI] | Δ vs Cook |
|---|---:|---:|---:|---:|
| **With-2016 train (`train=[2016, 2022]`)** | | | | |
| T-110 (≈ Jul 18) | 0.911 [0.65, 1.00] | 0.310 [0.00, 0.61] | 0.900 [0.70, 1.00] | +0.011 |
| T-60 (≈ Sept 6) | 0.907 [0.61, 1.00] | 0.267 [0.00, 0.63] | 0.900 [0.60, 1.00] | +0.007 |
| T-20 (≈ Oct 16) | 0.910 [0.53, 1.00] | 0.464 [0.17, 0.79] | 0.900 [0.70, 1.00] | +0.010 |
| **No-leak train (`train=[2022]` only — 2016 ratings are back-filled)** | | | | |
| T-110 | 0.909 [0.63, 1.00] | 0.310 [0.00, 0.61] | 0.900 [0.70, 1.00] | +0.009 |
| T-60  | 0.913 [0.53, 1.00] | 0.267 [0.00, 0.63] | 0.900 [0.60, 1.00] | +0.013 |
| T-20  | 0.730 [0.42, 1.00] | 0.464 [0.17, 0.79] | 0.900 [0.70, 1.00] | -0.170 |

**Interpretation:** at T-110 and T-60, model and Cook are statistically tied (CIs heavily overlap; the +1pp point-estimate lift sits well within the bootstrap noise floor). The honest claim is "model holds even with Cook's final ratings using inputs from 53 days earlier," not "+60pp." With-2016 training is robust at T-20 (Cook ties, 0.91 vs 0.90); no-leak training degrades at T-20 (smaller training set hurts late-cycle prediction more than mid-cycle), suggesting that the 2016 back-fill helps via larger N, not via the leaked rating quality.
